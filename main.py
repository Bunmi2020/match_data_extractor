...
def main():
    with sync_playwright() as p:
        try:
            ...
            # Base URL
            page.goto("https://www.soccer24.com/england/premier-league/fixtures/", timeout=19000000)
            page.wait_for_timeout(30000)  # Wait for the page to load completely
            ...
            if round_7_div:
                ...
                for link in links:
                    ...
                    try:
                        ...
                        page.wait_for_timeout(20000)  # Wait for H2H section to load
                        ...
                        if h2h_section:
                            ...
                            for row in h2h_rows:
                                ...
                                event_element = row.query_selector('span.h2h__event')
                                
                                if date_element and event_element:
                                    date_text = date_element.inner_text().strip()
                                    event_text = event_element.inner_text().strip()
                                    
                                    # Assuming date format is DD.MM.YY
                                    try:
                                        year = int(date_text.split('.')[-1])
                                        
                                        # Filter for years 2020 and beyond, and ignore "CF" events
                                        if year >= 20 and event_text != "CF":
                                            filtered_h2h_rows.append(row)
                                            print(f"Including match from year: {year}, event: {event_text}")
                                        else:
                                            print(f"Excluding match from year: {year}, event: {event_text}")
                                    
                                    except ValueError:
                                        print(f"Invalid date format: {date_text}")
                                else:
                                    print("Date or event element not found in h2h__row.")
                            ...
                            for idx, row in enumerate(filtered_h2h_rows[:5], start=1):
                                ...
                                try:
                                    ...
                                    stats_button = new_page.wait_for_selector(stats_button_selector, timeout=20000)
                                    if stats_button:
                                        stats_button.click()
                                        print("Clicked on the Stats button.")

                                        # Optionally, wait for the Stats section to load
                                        new_page.wait_for_timeout(20000)  # Adjust the timeout as needed
                                            ...
                                            if home_value_element and away_value_element:
                                                ...
                                                if category == "Expected Goals (xG)":
                                                    try:
                                                        ...
                                                        fixture_data.head_to_head.x_goals[match_key] = [0,0]
                                                    ...
                                                    try:
                                                        ...
                                                        fixture_data.head_to_head.corners[match_key] = [0,0]
                                                    ...
                                                    try:
                                                        ...
                                                        fixture_data.head_to_head.fouls[match_key] = [0,0]

                                               # Extract yellow and red cards and compute FT_cards
                                                if category in ["Yellow Cards", "Red Cards"]:
                                                    try:
                                                        # Initialize yellow and red cards to 0
                                                        home_yellow_cards = home_red_cards = away_yellow_cards = away_red_cards = 0

                                                        # Process yellow cards
                                                        if category == "Yellow Cards":
                                                            home_yellow_cards = int(home_value_text)
                                                            away_yellow_cards = int(away_value_text)

                                                        # Process red cards
                                                        elif category == "Red Cards":
                                                            home_red_cards = int(home_value_text)
                                                            away_red_cards = int(away_value_text)

                                                        # Ensure FT_cards is initialized with two elements [home, away]
                                                        if match_key not in fixture_data.head_to_head.FT_cards or len(fixture_data.head_to_head.FT_cards[match_key]) != 2:
                                                            fixture_data.head_to_head.FT_cards[match_key] = [0, 0]  # Initialize with [0, 0] for home and away

                                                        # Update FT_cards by adding yellow and red cards
                                                        fixture_data.head_to_head.FT_cards[match_key][0] += home_yellow_cards + home_red_cards  # Home team total cards
                                                        fixture_data.head_to_head.FT_cards[match_key][1] += away_yellow_cards + away_red_cards  # Away team total cards

                                                        print(f"FT_cards = {fixture_data.head_to_head.FT_cards[match_key]}")

                                                    except ValueError:
                                                        print(f"Error converting card values: '{home_value_text}', '{away_value_text}'")
                                                        fixture_data.head_to_head.FT_cards[match_key] = [0, 0]
                                ...
                                # Extract HT_cards (Yellow and Red Cards in 1st Half)
                                try:
                                    # Wait for the "1st Half" button to appear and click it
                                    first_half_button_selector = 'a[title="1st Half"] button[data-testid="wcl-tab"]'
                                    first_half_button = new_page.wait_for_selector(first_half_button_selector, timeout=20000)
                                    
                                    if first_half_button:
                                        first_half_button.click()
                                        print("Clicked on the 1st Half button.")
                                        
                                        new_page.wait_for_timeout(20000)  # Wait for 1st Half stats to load

                                        # Extract 1st Half statistics for Yellow and Red Cards
                                        first_half_stats_rows = new_page.query_selector_all('div._row_18zuy_8[data-testid="wcl-statistics"]')

                                        home_ht_yellow_cards = home_ht_red_cards = away_ht_yellow_cards = away_ht_red_cards = 0
                                        ...
                                        for fh_stat_row in first_half_stats_rows:
                                            ...
                                            if fh_category == "Yellow Cards":
                                                fh_home_value_element = fh_stat_row.query_selector('div._homeValue_7ptpb_9 strong')
                                                fh_away_value_element = fh_stat_row.query_selector('div._awayValue_7ptpb_13 strong')

                                                if fh_home_value_element and fh_away_value_element:
                                                    home_ht_yellow_cards = int(fh_home_value_element.inner_text().strip())
                                                    away_ht_yellow_cards = int(fh_away_value_element.inner_text().strip())

                                            if fh_category == "Red Cards":
                                                fh_home_value_element = fh_stat_row.query_selector('div._homeValue_7ptpb_9 strong')
                                                fh_away_value_element = fh_stat_row.query_selector('div._awayValue_7ptpb_13 strong')

                                                if fh_home_value_element and fh_away_value_element:
                                                    home_ht_red_cards = int(fh_home_value_element.inner_text().strip())
                                                    away_ht_red_cards = int(fh_away_value_element.inner_text().strip())

                                        # Combine yellow and red cards for HT_cards
                                        fixture_data.head_to_head.HT_cards[match_key] = [
                                            home_ht_yellow_cards + home_ht_red_cards,
                                            away_ht_yellow_cards + away_ht_red_cards
                                        ]
                                        print(f"HT_cards = {fixture_data.head_to_head.HT_cards[match_key]}")

                                    else:
                                        print("1st Half button not found.")
                                        fixture_data.head_to_head.HT_cards[match_key] = [0, 0]
                                except PlaywrightTimeoutError:
                                ...
                                page.wait_for_timeout(20000)  # Wait for the H2H section to reload
